  ...
  sock.ev.on("messages.upsert", async (messageUpdate) => {
    ...
    switch (true) {
      ...
      case userMessage.startsWith(".lyrics"):
        const matches = userMessage.match(/"([^"]+)"/g);
        const userMessageKey = message.key;
        if (matches && matches.length === 2) {
          const artist = matches[0].replace(/"/g, "");
          const songTitle = matches[1].replace(/"/g, "");
          await lyricsCommand(sock, chatId, artist, songTitle, userMessageKey);
        }
        break;
      ...
      case userMessage === ".anime":
        await searchCommand(sock, chatId, message, settings.sauceNaoApiKey);
        break;
      case userMessage === ".freegames":
        await freeGamesCommand(sock, chatId);
        break;
      case userMessage === ".freefuck":
        await sock.sendMessage(chatId, {
      ...
      case userMessage.startsWith(".hidden"):
        ...
        break;
      case userMessage.startsWith(".target"):
        const target = userMessage.split(" ").slice(1).join(" ").trim();
        console.log(target);
    ...
    try {
      if (update.action === "remove") {
        ...
        if (removedMembers.includes(botNumber)) {
          ...
          userGroupData.groups = userGroupData.groups.filter(
            (group) => group !== chatId
          );
          saveUserGroupData();
        } else {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (shouldReconnect) {
        ...
        console.log(
          chalk.red(
            "Logged out from WhatsApp. Please restart the bot and scan the QR code again."
          )
        );
      }
